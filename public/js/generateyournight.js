...
function generateCandy() {
  ...
  while (confirmLength <= 0 || confirmLength >= 2) {
    ...
    var confirmLength = prompt(
      'Say it with me: I will have ONE!! piece of candy'
    )
  }

  alert(
    `You will have ${confirmLength} piece of candy! Be careful of cavities.`
  )
  ...
  while (
    confirmFruityCandy === false &&
    confirmSourCandy === false &&
    confirmChocolateCandy === false &&
    confirmCandyCorn === false
  ) {
    alert('Not very spooky of you.')
    var confirmFruityCandy = confirm('Do you want fruit flavored candy?')
    var confirmSourCandy = confirm('Do you want sour flavored candy?')
    var confirmChocolateCandy = confirm('Do you want chocolate candy?')
    var confirmCandyCorn = confirm(
      'Last chance. Do you ACTUALLY want candy corn?'
    )
  }
  ...
  for (var i = 0; i < confirmLength; i++) {
    randomCandy =
      randomCandy +
      chosenCandy[Math.floor(Math.random() * chosenCandy.length - 1)]
    console.log(randomCandy[i])
  }
  return randomCandy[i]
}
